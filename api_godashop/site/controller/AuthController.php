...
class AuthController
{
    function login()
    {
        ...
        // check email tồn tại
        if (!$customer) {
            $_SESSION['error'] = 'Email không tồn tại. Vui lòng thử lại';
            header('location: /');
            exit;
        }
        // Check mật khẩu phải đúng
        if (!password_verify($password, $customer->getPassword())) {
            $_SESSION['error'] = 'Không đúng mật khẩu. Vui lòng thử lại';
            header('location: /');
            exit;
        }
        //check account đã kích hoạt chưa
        if ($customer->getIsActive() == 0) {
            $_SESSION['error'] = 'Tài khoản chưa được kích hoạt. Vui lòng vào email để kích hoạt hoặc liên hệ với quản trị';
            header('location: /');
            exit;
        }
